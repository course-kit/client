...
export default function createApi(baseUrl, tokens, devMode) {
  ...
  return {
    ...
    async loadCourses (schoolId) {
      const res = await get(`${baseUrl}/schools/${schoolId}`)
      return {
        status: res.status,
    ...
    async loadCourse (courseId) {
      const res = await get(`${baseUrl}/courses/${courseId}`)
    ...
    async loadLesson (courseId, lessonId) {
      const res = await get(`${baseUrl}/courses/${courseId}/lessons/${lessonId}`)
      if (devMode ? true : isPublished(res.data)) {
        return res
      } else {
        return {
