...
export default function (baseUrl, schoolId, devMode, api) {
  return {
    store: {
      user: null,
      courses: [],
    ...
    getLesson(courseId, lessonId) {
      ...
      if (lesson) {
        if (lesson.response.status === 404) {
          return { status: 404, lesson: null }
        }
        if (this.store.user && this.store.user.status === 200) {
          const user = this.store.user.data
          return createLesson(lesson.response, courseId, user, api)
        } else {
          return createLesson(lesson.response, courseId, null, api)
        }
      } else {
        return null
      }
