...
class Lesson {
  ...
  async loadPlayer(targetSelector, opts = {}) {
    const response = await this.loadFromServer()
    if (response.status === 200) {
      try {
        const { video } = response.data
        if (video) {
          const playerOptions = video
          playerOptions.autoplay = opts.autoplay
          playerOptions.hideControls = opts.hideControls
          playerOptions.showSubtitles = opts.showSubtitles
          return {
      ...
      if (typeof fetch === 'undefined') {
        ...
        try {
          ...
          if (res.status === 200) {
            ...
            response = { status: res.status, user: this.buildUser(data) }
          } else if (res.status !== 200 && res.status !== 500) {
            response = { status: res.status, user: this.buildUser() }
          }
        } catch (err) {
          console.log(err)
          response = { status: 500, user: this.buildUser() }
